...
AlarmController2::AlarmController2(Controllers::DateTime& dateTimeController) : dateTimeController {dateTimeController} {
}

namespace {
  void SetOffAlarm(TimerHandle_t xTimer) {
    auto* controller = static_cast<Pinetime::Controllers::AlarmController2*>(pvTimerGetTimerID(xTimer));
    controller->SetOffAlarmNow();
  }
}

void AlarmController2::Init(System::SystemTask* systemTask) {
  this->systemTask = systemTask;
  alarmTimer = xTimerCreate("Alarm2", 1, pdFALSE, this, SetOffAlarm);
}

void AlarmController2::SetAlarmTime(uint8_t alarmHr, uint8_t alarmMin) {
  hours = alarmHr;
  minutes = alarmMin;
}

void AlarmController2::ScheduleAlarm() {
...
uint32_t AlarmController2::SecondsToAlarm() const {
  return std::chrono::duration_cast<std::chrono::seconds>(alarmTime - dateTimeController.CurrentDateTime()).count();
}

void AlarmController2::DisableAlarm() {
  xTimerStop(alarmTimer, 0);
  state = AlarmState::Not_Set;
}

void AlarmController2::SetOffAlarmNow() {
  state = AlarmState::Alerting;
  systemTask->PushMessage(System::Messages::SetOffAlarm2);
}

void AlarmController2::StopAlerting() {
